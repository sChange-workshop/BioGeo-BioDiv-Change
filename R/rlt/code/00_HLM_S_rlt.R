...
#cell_count_alt <- rarefied_medians %>%
#	group_by(REALM, climate_mod, taxa_mod) %>%
#	dplyr::summarise(n_cells = n_distinct(rarefyID)) %>%
#	ungroup() %>%
#	unite(rlm_clm_txa, REALM, climate_mod, taxa_mod, remove=FALSE)
...
S_pois_RCTSRfyID <- brm(bf(S ~ cYEAR + (cYEAR|rlm_clm_txa/STUDY_ID/rarefyID), 
...
save(S_pois_RCTSRfyID, file=Sys.getenv('OFILE'))
