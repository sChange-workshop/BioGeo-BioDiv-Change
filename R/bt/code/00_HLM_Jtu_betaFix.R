...
rarefied_medians <- rarefied_medians %>%
        filter(n_cells > 3) %>%
	# add dirty fix for 0's and 1's
	mutate(Jtu.fix = ifelse(Jtu_base==0, Jtu_base + 1e-6, Jtu_base),
               Jtu.fix = ifelse(Jtu.fix==1, Jtu.fix - 1e-3, Jtu.fix))

##	set some priors...
hier_prior <- c(set_prior(prior = 'normal(0,0.05)', class='b', coef='cYEAR'), 	# global slope
...
##	stan via brms
Jtu_beta_BTSRfyID_countData <- brm(bf(Jtu.fix ~ cYEAR + (cYEAR|Biome/taxa_mod/STUDY_ID/rarefyID),
	#zoi ~ REALM, coi ~ REALM, 
...
save(Jtu_beta_BTSRfyID_countData, file=Sys.getenv('OFILE'))
