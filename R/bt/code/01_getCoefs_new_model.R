...
##  break into biome and rarefyID estimates
Jtu_norm_new_biome <- as_tibble(Jne_new_norm_ALL_groupCoefs[[1]]) %>%
...
# put 'em in the existing dataframes of coefs and ranefs
...
BT_global_estimates <- BT_global_estimates %>% 
  # remove the rows that have the parameter estimates of the model fit to all the data
  filter(model!='Jne_new_norm') %>% 
  bind_rows(Jne_new_norm_ALL_global %>% 
              mutate(term = c('Intercept', 'Slope')))

BT_biome_estimate <- BT_biome_estimate %>% 
  # remove the rows that have the parameter estimates of the model fit to all the data
  filter(model!='Jne_new_norm') %>% 
  bind_rows(Jtu_norm_new_biome)

BT_taxa_estimate <- BT_taxa_estimate %>%
  # remove the rows that have the parameter estimates of the model fit to all the data
  filter(model!='Jne_new_norm') %>% 
  bind_rows(Jne_new_norm_ALL_taxa)

BT_studyID_estimate <- BT_studyID_estimate %>% 
  # remove the rows that have the parameter estimates of the model fit to all the data
  filter(model!='Jne_new_norm') %>% 
  bind_rows(Jne_new_norm_ALL_studyID)

BTRfyID_rarefyID_coef <- BTRfyID_rarefyID_coef %>% 
  # remove the rows that have the parameter estimates of the model fit to all the data
  filter(model!='Jne_new_norm') %>% 
  bind_rows(Jne_new_norm_ALL_rarefyID)

BT_biome_ranef <- BT_biome_ranef %>% 
  # remove the rows that have the parameter estimates of the model fit to all the data
  filter(model!='Jne_new_norm') %>% 
  bind_rows(Jne_new_norm_ALL_biome_ranef)

BT_Taxa_ranef <- BT_Taxa_ranef %>% 
  # remove the rows that have the parameter estimates of the model fit to all the data
  filter(model!='Jne_new_norm') %>% 
  bind_rows(Jne_new_norm_ALL_Taxa_ranef)

BT_STUDYID_ranef <- BT_STUDYID_ranef %>% 
  # remove the rows that have the parameter estimates of the model fit to all the data
  filter(model!='Jne_new_norm') %>% 
  bind_rows(Jne_new_norm_ALL_studyID_ranef)

BT_rarefyID_ranef <- BT_rarefyID_ranef %>% 
  # remove the rows that have the parameter estimates of the model fit to all the data
  filter(model!='Jne_new_norm') %>% 
  bind_rows(Jne_new_norm_ALL_rarefyID_ranef)

## fix the column names for the study levels dfs (brms changed naming convention?)
BT_STUDYID_ranef <- BT_STUDYID_ranef %>% 
  mutate(Slope_ranef = Estimate,
         Slope_error = Est.Error,
         lower_slope = Q5,
         upper_slope = Q95) %>% 
  select(-Estimate, -Est.Error, -Q5, -Q95)

save(BT_global_estimates, BT_biome_estimate, BT_taxa_estimate, BT_studyID_estimate, BTRfyID_rarefyID_coef,
     BT_biome_ranef, BT_Taxa_ranef, BT_STUDYID_ranef, BT_rarefyID_ranef,
     file = '~/Dropbox/1current/BioTime/local_code/hierarchical/6results/bt/model_coefs_ranefs/BTRfyID_coef_ranef_inclNewModel.Rdata')
